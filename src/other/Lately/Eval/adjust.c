...
char **DropSplit(const char *_Ptr, const char *_T)
{
	...
	int i = 0, t = _strlen_asc(_T), f;
	char **_ret_;
	
	for (s1 = _Ptr; (s2 = _strstr_asc(s1, _T)) != NULL; i++) {
...
BOOL DropRegexGlobal(const char *ptr, const char *chr)
{
	...
	while ( ch = **ptrptr ) {
		...
		switch (ch) {
		case '\\':
			if (!AdjustCharacter(chrptr, *(*ptrptr + 1)))
				return FALSE;
			*ptrptr += 2;
			break;
		case '[':
