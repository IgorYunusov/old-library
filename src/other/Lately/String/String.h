...
namespace basm {
	...
	//
	//	Const String Array Class
	//
	template<class _ptr> class
		StringRefer
		{ // ConstArray Class
		typedef typename StringRefer<_ptr> this_type;
	protected:
		_ptr *_Ptr;
		size_t _Size;
	...
	public:
		...
		StringRefer(_ptr *_Str, size_t _Size)
			: _Ptr(_Str), _Size(_Size)
			{
			}
		...
		// increment by size_t
		this_type operator+=(size_t _Off) const
			{
			_Ptr += _Off;
			return *this;
			}
		...
		// subcript
		_ptr operator[](size_t _Off) const
			{
			return (*(*this + _Off));
			}
