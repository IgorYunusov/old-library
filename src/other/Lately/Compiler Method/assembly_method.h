...
std::vector<std::string> _assembly_x86_macrostyle(std::vector<std::string> intercode)
	{
	std::vector<std::string> var;
	int ebp_set = 8, dbp_sub=0, ebp_get=4;
...
#define _AM_T(name) if (map[name] == "eax")\
					eax_c = false;\
				else if (map[name] == "ecx")\
					ecx_c = false;\
				else if (map[name] == "edx")\
					edx_c = false;\
				else\
					ebx_c = false;
	...
	for(auto temp : intercode) {
		...
		if (is_variable(tsplit[1])) {
			if(!eax_c) {
				frame.push_back("mov eax, [ebp+" + std::to_string(ebp_set) + "]");
				map.insert(std::map<std::string, std::string>::value_type(tsplit[0], "eax"));
				eax_c = true;
			} else if(!ecx_c && !in_loop) {
				frame.push_back("mov ecx, [ebp+" + std::to_string(ebp_set) + "]");
				map.insert(std::map<std::string, std::string>::value_type(tsplit[0], "ecx"));
				ecx_c = true;
			} else if(!edx_c) {
				frame.push_back("mov edx, [ebp+" + std::to_string(ebp_set) + "]");
				map.insert(std::map<std::string, std::string>::value_type(tsplit[0], "edx"));
				edx_c = true;
			} else if(!ebx_c) {
				frame.push_back("mov ebx, [ebp+" + std::to_string(ebp_set) + "]");
				map.insert(std::map<std::string, std::string>::value_type(tsplit[0], "ebx"));
				ebx_c = true;
			} else {
			...
			if (is_number(second)) {
				frame.push_back("mov eax, " + second);
				if (map[first] != split(split(frame[frame.size() - 1], " ")[1], ",")[0])
					frame.push_back(std::string(assembly_text[_operator_x]) + " eax, " + map[first]);
				else {
					frame.push_back(std::string(assembly_text[_operator_x]) + " eax, " + split(frame[frame.size() - 1], " ")[2]);
					frame.erase(frame.begin()+frame.size()-2);
				}
				_AM_T(first)
			} else if (is_number(first)) {
				frame.push_back("mov eax, " + first);

				if (map[second] != split(split(frame[frame.size() - 1], " ")[1], ",")[0])
					frame.push_back(std::string(assembly_text[_operator_x]) + " eax, " + map[second]);
				else {
					frame.push_back(std::string(assembly_text[_operator_x]) + " eax, " + split(frame[frame.size() - 2], " ")[2]);
					frame.erase(frame.begin()+frame.size()-2);
					frame.erase(frame.begin()+frame.size()-2);
					frame.insert(frame.begin()+frame.size()-1, "mov eax, " + first);
				}
				_AM_T(second)
			} else {
				if(map[first] != "eax")
			...
			map.insert(std::map<std::string, std::string>::value_type(tsplit[0], "[ebp-" + std::to_string(ebp_get) + "]"));
			frame.push_back("mov [ebp-" + std::to_string(ebp_get) + "], " + "eax");
			ebp_get += 4;
		}
	}
	if (ebp_get>4)
		var.push_back("sub esi, " + std::to_string(ebp_get));
	for(auto temp : frame)
		var.push_back(temp);
...
std::vector<std::string> _assembly_x87_macrostyle(std::vector<std::string> intercode)
	{
	std::vector<std::string> var;
	int ebp_set = 4;
