...
public:
	...
	pointer allocate(size_type __n, const void* = 0) {
		if (__n > this->max_size()) throw "bad_alloc!";
...
public:
	...
	_Tree_allocator() _USE_NOEXCEPT { }
	_Tree_allocator(const _Tree_allocator<_Ty>& _Val) _USE_NOEXCEPT
		: _Normal_allocator_base<_Ty>(_Val) { }
	template<typename _Ty1>
	_Tree_allocator(const _Tree_allocator<_Ty1>&) _USE_NOEXCEPT { }
	~_Tree_allocator() _USE_NOEXCEPT { }

	template<typename _Ty1>
		struct rebind {
			typedef _Tree_allocator<_Ty1> other; };

	template<typename _Ty1>
		_Tree_allocator<_Ty>& operator=(const _Tree_allocator<_Ty1>&)
			{ return *this; }
...
template<typename _TyLeft, typename _TyRight>
	inline bool operator==(const _Tree_allocator<_TyLeft>&, const _Tree_allocator<_TyRight>&)
	{ return true; }

template<typename _TyLeft, typename _TyRight>
	inline bool operator!=(const _Tree_allocator<_TyLeft>& _Left, const _Tree_allocator<_TyRight>& _Right)
	{ return !(_Left == _Right); }

template<typename _Typ>
	using t_allocator = _Tree_allocator < _Typ > ;

struct _Container_base;
struct _Container_proxy
	{
	_Container_proxy() : _Mybase(0) {}

	_Container_base *_Mybase;
	};

struct _Container_base
	{
public:
	_Container_base() : _Myproxy(0) { }
	_Container_base(const _Container_base&) : _Myproxy(0) { }
	_Container_base& operator=(const _Container_base&) { return *this; }
	~_Container_base() _USE_NOEXCEPT { }
