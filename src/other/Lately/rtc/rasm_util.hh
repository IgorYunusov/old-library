...
void delete_in(std::vector<int>& user, int dest)
{
...
std::vector<_ty>& get_dup(std::vector<_ty> left, std::vector<_ty> right)
{
	std::vector<_ty> ret;
	std::copy_if(left.begin(), left.end(), ret.begin(), 
		[&](_ty s)->bool {
			for (auto tmp : right)
				if (tmp == s)
					return true;
...
bool if_in(std::vector<unsigned long>& x, unsigned long that)
{
	for (auto x_temp : x)
		if (x_temp == that)
			return true;
