 ...
 void  AddRoundKeyTo( uint32 * pInput , int iKeyIdx, uint32 * AesEncKey, uint32 * pOutput)
{
	...
	__asm {
		...
		mov ebx,DWORD PTR pRoundKey
		mov ecx,DWORD PTR pOutput
...
#if 0
	// 用 2-dimension array TestTableIdx
	ALIGN uint32 a1,a2,a3,a4;
	ALIGN uint32 state = temp[0];
...
#if 0
		// very slow if working with 64bit , impossible,just do table lookup is slow
...
//	ALIGN uint32 a1,a2,a3,a4,state;
	...
	SubAndShiftAndRound(temp, 40 ,pKey, pOutput);
}
