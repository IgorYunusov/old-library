	...
	for (int k = 0; k < a.getrows(); ++k) // here we get ones on the diagonal.  I am assuming that there will be no zeros on the diagonal, which is reasonable since this can not happen in this question.
	{
		...
		for (int m = 0; m < a.getcols(); ++m)
		{
			...
			if (k>m) {
				...
				if ((a[k][m]) != 0 && (a[k][m]) < 0.000001) (a[k][m]) = 0;
			};  // likewise, this is cosmetic, it just gets rid or tiny values which remain on the lower diagonal due to rounding errors, we're talking 10exp-16 small.  You can get rid of it if you like.
		}
