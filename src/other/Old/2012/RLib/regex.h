...
public:
	...
	INLINE  _Ty2A *_check_match(_Ty2A *_Left, _Ty2A *_Right)
		{ // ptr find match
	...
	INLINE _Ty2A *_check_match_n(_Ty2A *_Left, _Ty2A *_Right)
		{ // ptr find match
		_Ty2A *_Ptr = _Left, *_Sptr = _Right;
		for(_Ty2A *_N; *_Ptr; _Ptr++)
			if(*_Ptr == *_Sptr)
...
public:
	...
	FORCE_INLINE bool _Level1Engine_nextprocess(_Ty2A _Now_Characters, // check for pointer
		...
		case _META_CHECK _Meta_chr:
			...
			else if(_Now_Characters == *_LastPoint)
				return true;
